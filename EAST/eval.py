...
def detect(score_map, geo_map, timer, score_map_thresh=0.05, box_thresh=0.001, nms_thres=0.001):
    ...
    if len(score_map.shape) == 4:
        score_map = score_map[0, :, :, 0]#168*320,图像分辨率
        geo_map = geo_map[0, :, :, ]
                ...
                if boxes is not None:
                    ...
                    with open(res_file, 'w') as f:
                        for box in boxes:
                            ...
                            cv2.polylines(im[:, :, ::-1], [box.astype(np.int32).reshape((-1, 1, 2))], True, color=(0, 0, 255), thickness=5)
